...
#of the memory block we want to allocate (->rdi)
#
#RETURN VALUE:
...
no_init: 
    pushq %rdi
    leaq message_malloc(%rip), %rdi
    movl	$0, %eax
    call printf@PLT 
    popq %rdi
      
    movq %rdi, %rcx #%rcx will hold the size
                                 #we are looking for (which is the first
                                 #and only parameter)
    movq heap_begin(%rip), %rax       #%rax will hold the current
                                #search location
    movq current_break(%rip), %rbx    #%rbx will hold the current
                                #break
...
move_break:                     #return from the function
    ...
    movq %rbx, current_break(%rip) #save the new break
...
#we want to return to the memory pool (in rdi)
#
#RETURN VALUE:
